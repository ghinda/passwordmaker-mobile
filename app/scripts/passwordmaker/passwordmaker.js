/* PasswordMaker Generator Library
 */

var passwordmaker = (function() {

  var defaultParams = {
    // settings for the URL parsing
    url_protocol: false,
    url_subdomain: false,
    url_domain: true,
    url_path: false,
  ...
  var generateCharacter = function(hashAlgorithm, key, data, whereToUseL33t, l33tLevel, charset) {
  ...
  var splitSubdomain = function(segments) {
  ...
  var generate = function(params) {
    ...
    if (params.selectedCharset.length < 2) {
      return 'Not enough chars!';
    }

    params.username = params.username || '';

    if(params.strUseText) {
      // fixed text/url
      params.url = params.strUseText;
    } else {
    ...
    while (password.length < params.passwordLength) {
      ...
      if (count === 0) {
        password += generateCharacter(params.hashAlgorithm, params.masterkey,
            params.url + params.username + params.modifier, params.whereToUseL33t, params.l33tLevel, params.selectedCharset);
      } else {
        password += generateCharacter(params.hashAlgorithm, params.masterkey + '\n' + count,
            params.url + params.username + params.modifier, params.whereToUseL33t, params.l33tLevel, params.selectedCharset);
      }
    ...
    if (params.passwordPrefix) {
      password = params.passwordPrefix + password;
    }
    if (params.passwordSuffix) {
      password = password.slice(0, params.passwordLength - params.passwordSuffix.length) + params.passwordSuffix;
    }

    return password.slice(0, params.passwordLength);
  };
